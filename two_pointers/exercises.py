...
def triplet_sum_close_to_target(arr, target_sum):
    ...
    for i in range(len(arr) - 2):
        ...
        while left < right:
            ...
            computed_distance = target_sum - arr[i] - arr[left] - arr[right]

            if computed_distance == 0:

                return computed_distance
            
            if (abs(smallest_distance) > abs(computed_distance)) or ((abs(smallest_distance) == abs(computed_distance)) and (computed_distance > smallest_distance)):
